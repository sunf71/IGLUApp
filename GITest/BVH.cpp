...
bool BVH::getIntersection(const Ray& ray, IntersectionInfo* intersection, bool occlusion) const {
 ...
 while(stackptr>=0) {
  ...
  //float h1,h2;
  //if ( !flatTree[ni].bbox.intersect(ray, &h1, &h2) )
	 // continue;
...
void BVH::frustumCulling(Frustum& frustum, std::vector<int>& passedIdx) const
{
	//Working set
	BVHTraversal todo[64];
	int32_t stackptr = 0;
	...
	while(stackptr>=0) {
		...
		if( node.rightOffset == 0 ) {
			for(uint32_t o=0;o<node.nPrims;++o) {
				...
				if (frustum.ContainsTriangle(tri) != Out)
